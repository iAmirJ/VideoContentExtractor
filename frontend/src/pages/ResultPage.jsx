...
const ExportCard = ({icon, title, onClick}) => (
  <div className="export-card-hover" style={{border:'1px solid #e2e8f0', padding:'25px 20px', borderRadius:'10px', width:'130px', cursor:'pointer', transition: 'all 0.2s', background: '#f8fafc'}} onClick={onClick}>
...
const ResultPage = ({ activeTab }) => {
  ...
  const handleExport = async (format) => {
    ...
    try {
      ...
      const response = await axios.post(`http://127.0.0.1:8000/api/export/${format}`, {
        title: "VidioMind Summary",
        ...
        <div className="card" style={{ borderTop: 'none', borderRadius: '0 0 12px 12px', padding: '30px', minHeight: '400px' }}>
            ...
            {activeTab === 'export' && (
                <div style={{textAlign:'center', paddingTop:'50px'}}>
                    ...
                    {exporting && <p style={{color: '#2563eb', marginTop: '10px'}}><Loader2 className="spin" size={16} style={{display:'inline', verticalAlign:'middle'}}/> Generating your file...</p>}
